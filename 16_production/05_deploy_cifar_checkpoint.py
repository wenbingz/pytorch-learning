"""
CIFAR-10 checkpoint -> ONNX -> ONNXRuntime (L5 or L16 production ckpt).

Train first (pick one):
...
def eval_onnx(session, loader: DataLoader) -> tuple[float, float]:
    ...
    for xb, yb in loader:
        ...
        correct += (logits_t.argmax(dim=1) == yb).sum().item()
        total += yb.size(0)
    return total_loss / total, correct / total
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Deploy CIFAR-10 checkpoint via ONNXRuntime")
    parser.add_argument("--ckpt", default="runs/cifar10/ckpt.pt")
    parser.add_argument(
        "--weights",
        default="model",
        choices=["model", "ema"],
        help="use training weights or EMA shadow weights (production ckpt)",
    )
    parser.add_argument("--data-dir", default="data/cifar10")
    parser.add_argument("--out-dir", default="runs/deployment/cifar10")
...
def main() -> None:
    ...
    if not ckpt_path.exists():
        ...
        print("train first:")
        print("  python3 05_cnn_cifar10/train_cifar10.py --epochs 20")
        print("  python3 16_production/04_train_cifar_production.py --epochs 20")
        return

    ckpt = load_checkpoint(ckpt_path, map_location="cpu")
    weight_key = args.weights
    if weight_key not in ckpt:
        print(f"checkpoint has no '{weight_key}' key.")
        if weight_key == "ema":
            print("train with 04_train_cifar_production.py or pass --weights model")
        return

    acc_key = "best_ema_acc" if weight_key == "ema" else "best_test_acc"
    best_acc = float(ckpt.get(acc_key, ckpt.get("best_val_acc", float("nan"))))

    model = CIFAR10CNN().eval()
    model.load_state_dict(ckpt[weight_key])

    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    suffix = "ema" if weight_key == "ema" else "model"
    onnx_path = out_dir / f"cifar10_cnn_{suffix}.onnx"

    print("step 1/4: loaded checkpoint")
    print(f"  path: {ckpt_path}")
    print(f"  weights: {weight_key}")
    print(f"  best acc in ckpt: {best_acc:.4f}")
    ...
    print()
    print(f"deployment done: serve {onnx_path} with onnxruntime")
